...
from ldapManager.ldapApi import MyLdap
from tools.config import ldapHeader
import json
ldapUrl = Blueprint('ldap', __name__)


@ldapUrl.route('/api/v1', methods=['GET', 'POST', 'DELETE'])
def ldapRun():
    """
    1.路由入口方法
    """
    ...
    if request.method == "GET":
        ...
        if username or name or mail:
            ...
            return Response(json.dumps(
                {"code": 0, "total": len(queryData), "data": [i.to_dict() for i in ldapData], "columns": ldapHeader}),
    ...
    try:
        if username:
            ...
            if retData.get("code") == 0:
                ...
                return Response(json.dumps({"code": 1, "data": "删除数据Ldap账号失败"}), mimetype='application/json')
        else:
            parameterInfo = "参数不足或错误,请进行检查"
            return Response(json.dumps({"code": 1, "data": parameterInfo}), mimetype='application/json')
    except Exception as e:
...
def deletSQLLdap(id):
    ...
    try:
        ...
        data = {"code": 500, "data": "delete ldap faild", "message": str(e)}
        return Response(json.dumps(data), mimetype='application/json')
...
@ldapUrl.route('/scrapes/v1', methods=['GET', 'POST', 'DELETE'])
def scrapesLdapRun():
    if request.method == "GET":
        return scrapesLdap()
    else:
