...
def qeuryApp():
    ...
    if appname or apptype or giturl or port or owner or level or group:
        ...
        return Response(json.dumps(
            {"code": 0, "total": len(queryData), "data": [i.to_dict() for i in appData], "columns": appMgHeader}),
...
class DeployParameter(object):
    def releaseList(self):
        import json
        from tools.config import releaseListkey, releaseListDisplay_name
    ...
    def appListEnv(self):
        from tools.config import envListkey, envListdisplay_name
        import json
        Data = dict(zip(envListkey, envListdisplay_name))
        return json.dumps({"code": 0, "data": [{"key": x, "display_name": Data[x]} for x in Data]})

    def languageListEnv(self):
        from tools.config import languageType, languageTypedisplay_name
        import json
...
def searchPackListRun():
    from tools.config import searchPackPath
    import json
...
def searchServicesdir(path, server, branch):
    ...
    #     Data = dict(zip(server,server_dict ))
    # return json.dumps({"code": 0, "data":[{"key": server, "display_name": server_dict[x]} for x in server_dict]})
...
def dataResult(Data):
    ...
    #     "columns": appMgHeader
    # }
    return Response(json.dumps({"code": 0, "total": len(data_list),"data":data_list,"msg":msg,"columns":appMgHeader}),mimetype='application/json')
